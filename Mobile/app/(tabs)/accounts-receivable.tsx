...
export default function AccountsReceivableScreen({ isEmbedded = false }: { isEmbedded?: boolean }) {
  ...
  // Modal States
  const [selectedAccount, setSelectedAccount] = useState<AccountsReceivableDto | null>(null);
  ...
  const handleReceiveSubmit = () => {
    ...
    const amountNum = parseFloat(receiveAmount.replace(',', '.'));
    if (isNaN(amountNum) || amountNum <= 0) {
      Alert.alert('Erro', 'Informe um valor válido maior que zero.');
      return;
    }
    if (!receiveWalletId) {
      Alert.alert('Erro', 'Selecione uma carteira de destino.');
      return;
    }
    ...
    receiveMutation.mutate({
      ...
      bankAccountId: receiveBankAccountId || null,
    });
  };
  ...
  return (
    <Container style={isEmbedded ? { flex: 1 } : styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
          {accountsReceivable && accountsReceivable.length > 0 ? (
            accountsReceivable.map((item, index) => (
              ...
              >
                ...
                <View style={styles.cardFooter}>
                  ...
                  {item.isLate && (
                    <View style={[styles.badge, { backgroundColor: 'rgba(255, 179, 71, 0.1)' }]}>
            ...
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptyText}>Você não possui receitas pendentes para este período.</Text>
            </View>
          )}
        </ScrollView>
      )}
            ...
            <View style={styles.detailCard}>
              <View style={styles.formHeader}>
                <View style={styles.formHeaderInfo}>
                  <Text style={styles.formTitle}>Detalhes da Receita</Text>
                </View>
              ...
              <View style={styles.detailBody}>
                ...
                <View style={styles.detailField}>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.sm }}>
                    ...
                    {selectedAccount?.isLate && (
                      <View style={[styles.badge, { backgroundColor: 'rgba(255, 179, 71, 0.1)', alignSelf: 'flex-start' }]}>
                        <Ionicons name="alert-circle-outline" size={12} color={colors.warning} />
                        <Text style={[styles.badgeText, { color: colors.warning }]}>Pendente</Text>
                      </View>
                    )}
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    if (selectedAccount) {
                      setReceiveAmount(selectedAccount.amount.toString());
                      setReceiveWalletId('');
                ...
                >
                  ...
                  <Text style={styles.receiveActionBtnText}>Receber</Text>
                </TouchableOpacity>
              </View>
            </View>
          </SafeAreaView>
      ...
      {/* Receive Modal */}
      <Modal visible={isReceiveModalOpen} animationType="slide" transparent>
        <KeyboardAvoidingView
          ...
          <View style={styles.modalOverlay}>
            <SafeAreaView style={styles.formContainer}>
              <View style={styles.formCard}>
                <View style={styles.formHeader}>
                  <View style={styles.formHeaderInfo}>
                    <Text style={styles.formTitle}>Registrar Recebimento</Text>
                  </View>
                ...
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.formScrollBody}>
                  <View style={styles.fieldWrapper}>
                    <Text style={styles.label}>Receita Recorrente</Text>
                    <TextInput
                  ...
                  <View style={styles.fieldWrapper}>
                    <Text style={styles.label}>Destino do Valor (Carteira / Conta)</Text>
                    <TouchableOpacity style={styles.selectInput} onPress={() => {
                      setIsReceiveModalOpen(false);
                      setIsWalletSelectOpen(true);
                    }}>
                      <Text style={[styles.selectInputText, !receiveWalletId && { color: colors.text.muted }]}>
                        {receiveWalletId
                          ? (receiveBankAccountId 
                              ? `Conta Bancária - ${wallets?.find(w => w.id === receiveWalletId)?.accounts.find(a => a.id === receiveBankAccountId)?.bankName}`
                              : `Dinheiro Vivo - ${wallets?.find(w => w.id === receiveWalletId)?.name}`)
                          : 'Selecionar destino'}
                      </Text>
                  ...
                  >
                    {receiveMutation.isPending ? (
                      ...
                      <Text style={styles.saveBtnText}>Confirmar Recebimento</Text>
                    )}
                  </TouchableOpacity>
                </ScrollView>
              </View>
